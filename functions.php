...
function twp_enqueue_style($hook) {
	$pages_array = array('toplevel_page_telegram-for-wp');
	if (!in_array($hook, $pages_array)){
		return;
	}
	wp_register_style( 'twp_spectre_css', TWP_PLUGIN_URL. '/inc/css/spectre.min.css', false, '1.0.0' );
	wp_register_style( 'twp_admin_css', TWP_PLUGIN_URL. '/inc/css/twp-admin.css', false, '1.0.0' );

	wp_enqueue_style( 'twp_spectre_css' );
	wp_enqueue_style( 'twp_admin_css' );
}
add_action( 'admin_enqueue_scripts', 'twp_enqueue_style' );
/**
...
function twp_ajax_test_callback() {
	...
	switch ($_POST['subject']) {
		...
		//Send a test message using Notifcaster.
			...
			$result = $nt->sendmessage($_POST['chat_id'], $_POST['text']);
